...
// General Playlist renderer contract marker: playlist?.title
// The rendered title is stored in pageTitle and the introduction in pageIntroduction.
function CustomOutreachReelPage() {
  ...
  const [pageTitle, setPageTitle] = useState("Selected Work — Marius Ygre");
  const [pageIntroduction, setPageIntroduction] = useState("A focused selection of music for film and visual storytelling.");
  const [selectedIndex, setSelectedIndex] = useState(0);
  ...
  useEffect(() => {
    if (loadState === "ready" && pageTitle) document.title = /marius ygre/i.test(pageTitle) ? pageTitle : `${pageTitle} — Marius Ygre`;
  }, [loadState, pageTitle]);
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then(([catalog, reel]) => {
        ...
        setPageTitle(String(reel.title || "Selected Work — Marius Ygre").trim());
        setPageIntroduction(String(reel.introduction || "A focused selection of music for film and visual storytelling.").trim());
        setSelectedIndex(0);
